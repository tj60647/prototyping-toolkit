...
| Backend (TLDR Part 2) | Add steps 14–18 and five guides (2026-09-16) | Keys on the server, a Neon database per app with Neon's agent tools, Google sign-in through Neon Auth with an `ALLOWED_EMAILS` check in server code, a private Vercel Blob store, and a limits check. Design and research: `backend-guide-design.md`; plan: `backend-guide-plan.md`. A build check now fails if any TLDR prompt is missing from the detailed guides. |
| Where setup lives | Global, not per project (2026-09-16) | Installs say "for all my projects", except Superpowers, which students install **locally** to try on one project before installing it for all projects. The publishing rule goes in the assistant's global instructions (`~/.claude/CLAUDE.md` or `~/.codex/AGENTS.md`) instead of each project's `AGENTS.md`, and names no project. Tested 2026-09-16: Vercel CLI doesn't find the project from Git on its own (`vercel env ls` in an unlinked clone asks for `--project` or `vercel link`), but the agent resolves that itself, by passing the name or linking non-interactively, and Vercel's API can find a project by its GitHub repository (`/v10/projects?repoUrl=`). Students no longer note or type the project name. Only the prototyping instructions (step 10) and `.env.local` (step 8) stay per project. |
...
| TLDR Part 2 | Dry run on a real prototype (not yet done): no key in the built files; signed-out requests to the paid API refused; an email not on `ALLOWED_EMAILS` refused; uploads land in Blob and aren't publicly readable; Neon's agent tools install and sign in from the agent |
